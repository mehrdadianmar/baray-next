...
const post=(id,slug,title)=>({id,slug,date:"2026-08-16T09:00:00",modified:"2026-08-16T10:00:00",link:`https://example.test/${slug}/`,title:{rendered:title},excerpt:{rendered:`<p>خلاصه ${title}</p>`},content:{rendered:`<h2>${title}</h2><p>متن کامل خبر</p><script>alert(1)</script>`},_embedded:{author:[{name:"تحریریه تست"}],"wp:featuredmedia":[{source_url:pathToFileURL(mediaFile).href,alt_text:title}],"wp:term":[[{taxonomy:"category",name:"خبر تست"}]]}});
const fixture=path.join(temp,"fixture.json");await fs.writeFile(fixture,JSON.stringify([post(1,"first-news","خبر اول"),post(2,"second-news","خبر دوم")]));
