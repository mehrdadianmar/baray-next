...
for(const file of files){const html=documentOnly(fs.readFileSync(file,"utf8"));for(const match of html.matchAll(/href="(\/[^"]*)"/g)){const href=match[1].split("#")[0].split("?")[0];if(!href||href.startsWith("//")||href.includes("."))continue;const local=basePath&&href.startsWith(basePath)?href.slice(basePath.length)||"/":href;let decoded;try{decoded=decodeURIComponent(local)}catch{decoded=local}checks++;const target=decoded==="/"?path.join(root,"index.html"):path.join(root,decoded,"index.html");if(!fs.existsSync(target))failures.push(`${path.relative(root,file)}: broken ${href}`)}}
